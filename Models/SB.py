...
def Tower_layer(dat, keep_prob):
    ...
    H3 = tf.layers.dense(inputs=Drop_H2, units=n_output, activation=tf.nn.sigmoid)

    return H3

kp = tf.placeholder(tf.float32)
H1 = Tower_layer(concat_input, kp)
H2 = Tower_layer(concat_input, kp)
...
for epoch in range(config.epochs):
    train_loss,opt = sess.run((cost, optimizer), feed_dict=feed_dict)
    print("epoch: {0}, train loss: {1:.6f}".format(epoch, train_loss))

def random_mini_batches(Xi, Xv, R1E, R2E, mini_batch_size=500, seed=42):
    # Creating the mini-batches
    np.random.seed(seed)
    m = Xi.shape[0]
    mini_batches = []
    permutation = list(np.random.permutation(m))
    shuffled_Xi = Xi[permutation, :]
    shuffled_Xv = Xv[permutation, :]
    shuffled_X1R = R1E[permutation]
    shuffled_X2R = R2E[permutation]
    num_complete_minibatches = math.floor(m / mini_batch_size)
    for k in range(0, int(num_complete_minibatches)):
        mini_batch_Xi = shuffled_Xi[k * mini_batch_size: (k + 1) * mini_batch_size, :]
        mini_batch_Xv = shuffled_Xv[k * mini_batch_size: (k + 1) * mini_batch_size, :]
        ...
        mini_batch = (mini_batch_Xi, mini_batch_Xv, mini_batch_X1R, mini_batch_X2R)
        mini_batches.append(mini_batch)
    Lower = int(num_complete_minibatches * mini_batch_size)
    Upper = int(m - (mini_batch_size * math.floor(m / mini_batch_size)))
    if m % mini_batch_size != 0:
        mini_batch_Xi = shuffled_Xi[Lower: Lower + Upper, :]
        mini_batch_Xv = shuffled_Xv[Lower: Lower + Upper, :]
...
with tf.Session() as sess:
    ...
    for epoch in range(num_epochs):
        ...
        num_minibatches = int(len(Xi_train) / config.mb_size)
        minibatches = random_mini_batches(Xi_train, Xv_train, Y1_train, Y2_train, config.mb_size, seed)

        for minibatch in minibatches:
            (mb_Xi, mb_Xv, mb_Y1, mb_Y2) = minibatch
        ...
        if epoch % 1 == 0:
            ...
            Y1_pre, Y2_pre = sess.run([H1, H2], feed_dict = {
                                        feat_index: Xi_test,
                                        feat_value: Xv_test,
                                        Y1: Y1_test,
                                        Y2: Y2_test,
            ...
            print('Epoch: %d, Y1 AUC: %5.3f, Y2 AUC: %5.3f' % (
                epoch,
                roc_auc_score(Y1_test, Y1_pre),
                roc_auc_score(Y2_test, Y2_pre)
            ))
